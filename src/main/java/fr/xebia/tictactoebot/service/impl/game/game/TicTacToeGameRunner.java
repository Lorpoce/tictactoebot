package fr.xebia.tictactoebot.service.impl.game.game;
...
public class TicTacToeGameRunner {
	 ...
	 * 
	 * @param agent
	 *            a {@link GameIntelligenceAgent} for choosing the computer
	 *            opponent's moves
	 * @param scanner
	 *            a {@link Scanner} for collecting user input
	 * @param printStream
	 *            the {@link PrintStream} for displaying text to the user
	 */
	...
	/**
	 * Runs the TicTacToe game, alternating between human and computer moves
		...
		while (true) {
			...
			try {
				if (game.play(userPosition.getRow(), userPosition.getCol())) {
					...
					printStream.printf("(%d,%d) has already been taken. ",
							userPosition.getRow(), userPosition.getCol());
					printInstructions();
				}
			} catch (IllegalArgumentException e) {
				printStream.printf("(%d,%d) is not on the board. ",
						userPosition.getRow(), userPosition.getCol());
				printInstructions();
			}
		}
