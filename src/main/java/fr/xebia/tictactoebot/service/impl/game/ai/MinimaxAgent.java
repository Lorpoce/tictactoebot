package fr.xebia.tictactoebot.service.impl.game.ai;
...
import fr.xebia.tictactoebot.service.impl.game.ai.heuristic.StateEvaluator;
 ...
 */
public class MinimaxAgent<T extends DiscreteGameState>
		implements
			GameIntelligenceAgent<T> {
	...
	public T evaluateNextState(T currentState, int depth) {
		...
		if (depth < 0) {
			throw new IllegalArgumentException(
					"depth cannot be less than zero. depth=" + depth);
		}
	...
	private Node<T> buildTree(T state, boolean max, int alpha, int beta,
			int depth) {
		...
		if (depth == 0 || state.isOver()) {
			...
			for (DiscreteGameState nextState : state.availableStates()) {
				Node<T> child = buildTree((T) nextState, !max, current.alpha,
						current.beta, depth - 1);
	...
	private Node<T> buildTree(T state, int depth) {
		return buildTree(state, true, Integer.MIN_VALUE, Integer.MAX_VALUE,
				depth);
	}
}
